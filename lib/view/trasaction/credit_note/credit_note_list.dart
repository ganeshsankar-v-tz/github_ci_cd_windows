...
import 'package:abtxt/view/trasaction/credit_note/add_credit_note.dart';
...
import 'credit_note_filter.dart';

class CreditNoteList extends StatefulWidget {
  const CreditNoteList({super.key});

  static const String routeName = '/credit_note_list';

  @override
  State<CreditNoteList> createState() => _CreditNoteListState();
}

class _CreditNoteListState extends State<CreditNoteList> {
        ...
        return CoreWidget(
          ...
          child: MySFDataGridRawTable(
            ...
            columns: [
              ...
              GridColumn(
                columnName: 'credit_note_type',
                label: const MyDataGridHeader(title: 'Credit Note Type'),
              ),
              GridColumn(
                visible: false,
                width: 110,
  ...
  void _add({Map<String, dynamic>? args}) async {
    var result = await Get.toNamed(AddCreditNote.routeName, arguments: args);
    if (result == true) {
      _api(request: controller.filterData);
    }
  }
  ...
  Future<void> _api({var request = const {}}) async {
    var response = await controller.creditNoteList(request: request);
    list.clear();
  ...
  void _filter() async {
    final result = await showDialog(
      context: context,
      builder: (_) => const CreditNoteFilter(),
    );
    ...
    _employeeData = _list.map<DataGridRow>((e) {
      return DataGridRow(cells: [
        ...
        DataGridCell<dynamic>(
            columnName: 'credit_note_type', value: '${e["credit_note_type"]}'),
