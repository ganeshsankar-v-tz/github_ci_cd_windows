...
class ProductPurchaseController extends GetxController with StateMixin {
  List<FirmModel> firmName = <FirmModel>[].obs;
  List<LedgerModel> Supplier = <LedgerModel>[].obs;
  List<ProductInfoModel> ProductName = <ProductInfoModel>[].obs;
  List<LedgerModel> Account = <LedgerModel>[].obs;
  ...
  void onInit() async {
    ...
    change(Supplier = await supplierInfo());
    change(ProductName = await productInfo());
    change(Account = await purchaseAccount());
  ...
  // Customer Name
  Future<List<LedgerModel>> supplierInfo() async {
    print('api/ledger');
    List<LedgerModel> result = [];
    await HttpRepository.apiRequest(
