...
class WarpOrYarnDyingController extends GetxController with StateMixin {
  ...
  Future<dynamic> warpYarndying({var page = "1"}) async {
    dynamic result = [];
    await HttpRepository.apiRequest(
            HttpRequestType.get, 'api/warp_yarn_pagination?page=$page')
        .then((response) {
      if (response.success) {
        var json = jsonDecode(response.data);
        var data = json['data']['data'];
        var list = (data as List)
            .map((i) => WarpOrYarnDyingModel.fromJson(i))
            .toList();
        ...
        result = {"list": <WarpOrYarnDyingModel>[], "totalPage": 1};
      }
    });
    return result;
  }

  void addWarpYarn(var request) async {
    await HttpRepository.apiRequest(HttpRequestType.post, 'api/add_warp_yarn',
            requestBodydata: request)
        .then((response) {
      if (response.success) {
        var json = jsonDecode(response.data);
        print('addyarnreturn: $json');
        Get.back();
  ...
  void updateWarpYarn(var request) async {
    await HttpRepository.apiRequest(
            HttpRequestType.post, 'api/update_warp_yarn',
            requestBodydata: request)
        .then((response) {
      if (response.success) {
        var json = jsonDecode(response.data);
  ...
  Future<dynamic> deleteWarpYarn(var id) async {
    await HttpRepository.apiRequest(
            HttpRequestType.delete, 'api/delete_warp_yarn/$id')
        .then((response) {
      if (response.success) {
        var json = jsonDecode(response.data);
        AppUtils.showSuccessToast(message: "${json["message"]}");
