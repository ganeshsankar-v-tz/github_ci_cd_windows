import 'package:abtxt/view/trasaction/purchase_return/warp_purchase_return/warp_purchase_return_bottom_sheet.dart';
import 'package:abtxt/view/trasaction/purchase_return/warp_purchase_return/warp_purchase_return_controller.dart';
...
class AddWarpPurchaseReturn extends StatefulWidget {
  final Map? value;

  const AddWarpPurchaseReturn({
    super.key,
    this.value,
  });

  static const String routeName = '/add_warp_purchase_return';

  @override
  State<AddWarpPurchaseReturn> createState() => _State();
}

class _State extends State<AddWarpPurchaseReturn> {
  ...
  Rxn<WarpPurchaseReturnSlipNoDropdown> slipNoValue =
      Rxn<WarpPurchaseReturnSlipNoDropdown>();
  ...
  WarpPurchaseReturnController controller =
      Get.put(WarpPurchaseReturnController());
  ...
  Widget build(BuildContext context) {
    return GetBuilder<WarpPurchaseReturnController>(builder: (controller) {
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            MySearchField(
                              ...
                              textController: supplierController,
                            ...
                            MySearchField(
                              ...
                              onChanged:
                                  (WarpPurchaseReturnSlipNoDropdown value) {
  ...
  submit() async {
    if (_formKey.currentState!.validate()) {
      Map<String, dynamic> request = {
        "debit_note_type": "Warp purchase return",
  ...
  void _initValue() async {
    ...
    firmName.value = AppUtils.setDefaultFirmName(controller.firmNameDropdown);
    if (widget.value != null && widget.value!.isNotEmpty) {
      ...
      supplierController.text = "${item["supplier_name"]}";

      slipNoValue.value = WarpPurchaseReturnSlipNoDropdown(
          id: item["slip_no"], slipNo: item["slip_no"]);
      ...
      al1ano = int.tryParse(item['al1_ano'] ?? "0");
      al2ano = int.tryParse(item['al2_ano'] ?? "0");
      al3ano = int.tryParse(item['al3_ano'] ?? "0");
      al4ano = int.tryParse(item['al4_ano'] ?? "0");
  ...
  Widget itemsTable() {
    return MySFDataGridItemTable(
      ...
      onRowSelected: (index) async {
        ...
        var result = await Get.to(const WarpPurchaseReturnBottomSheet(),
            arguments: {'item': item});
        if (result != null) {
  ...
  void _apiCalls() async {
    ...
    controller.slipNoDetails(
        "Warp purchase return", firmName.value?.id, supplier.value?.id);
  }

  void _selectedSlipNoDisplay(WarpPurchaseReturnSlipNoDropdown item) {
