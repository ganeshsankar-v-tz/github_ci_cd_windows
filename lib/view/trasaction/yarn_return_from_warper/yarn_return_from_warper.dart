...
import 'package:abtxt/view/trasaction/yarn_return_from_warper/add_yarn_return_from_warper.dart';
import 'package:abtxt/view/trasaction/yarn_return_from_warper/yarn_return_from_warper_controller.dart';
import 'package:abtxt/view/trasaction/yarn_return_from_warper/yarn_return_from_warper_filter.dart';
...
class YarnReturnFromWarper extends StatefulWidget {
  const YarnReturnFromWarper({super.key});

  static const String routeName = '/YarnReturnFromWarper';

  @override
  State<YarnReturnFromWarper> createState() => _State();
}

class _State extends State<YarnReturnFromWarper> {
  YarnReturnFromWarperController controller =
      Get.put(YarnReturnFromWarperController());
  ...
  Widget build(BuildContext context) {
    return GetBuilder<YarnReturnFromWarperController>(builder: (controller) {
      return CoreWidget(
        appBar: AppBar(
          title: const Text('Transaction / Yarn Return From Warper'),
          actions: [
        ...
        child: MySFDataGridRawTable(
          ...
          columns: <GridColumn>[
            ...
            GridColumn(
              width: 100,
              columnName: 'pck',
              label: const MyDataGridHeader(title: 'Pck'),
            ),
            GridColumn(
              width: 100,
              columnName: 'quantity',
  ...
  void _api({var request = const {}}) async {
    var response = await controller.yarnReturnfromWarper(request: request);
    list.clear();
  ...
  void _add({Map<String, dynamic>? args}) async {
    var result =
        await Get.toNamed(AddYarnReturnFromWarper.routeName, arguments: args);
    if (result == 'success') {
      _api(request: controller.filterData ?? {});
    }
  }
  ...
  void _filter() async {
    final result = await showDialog(
      context: context,
      builder: (_) => const YarnReturnFromWarperFilter(),
    );
