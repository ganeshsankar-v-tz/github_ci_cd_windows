...
import '../../../model/yarn_sale_model.dart';
...
import 'add_yarn_sale_bottom_sheet.dart';

class AddYarnSales extends StatefulWidget {
  const AddYarnSales({super.key});

  static const String routeName = '/AddYarnSales';

  @override
  State<AddYarnSales> createState() => _State();
}

class _State extends State<AddYarnSales> {
  ...
  YarnSaleController controller = Get.find();
  ...
  Widget build(BuildContext context) {
    return GetBuilder<YarnSaleController>(builder: (controller) {
      return ShortCutWidget(
        appBar: AppBar(
          title:
              Text("${idController.text == '' ? 'Add' : 'Update'} Yarn Sale"),
          actions: [
            Visibility(
  ...
  Future<void> _print() async {
    ...
    String? response = await controller.yarnSalePdf(request: request);
  ...
  submit() async {
    if (_formKey.currentState!.validate()) {
      Map<String, dynamic> request = {
        ...
        "sales_date": salesDateController.text,
      ...
      if (id.isEmpty) {
        ...
        controller.addYarnsale(request);
      } else {
        request['id'] = id;
        request['bill_no'] = int.tryParse(billNoController.text);
        controller.updateYarnsale(request, id);
      }
    }
  }

  void _initValue() {
    salesDateController.text = AppUtils.parseDateTime("${DateTime.now()}");
    // lrController.text = AppUtils.parseDateTime("${DateTime.now()}");
    YarnSaleController controller = Get.find();
    freightController.text = Constants.freight[0];
    accountType.value = AppUtils.findLedgerAccountByName(
        controller.purchaseAccountDropdown, 'Yarn Sales Account');
    firmName.value = AppUtils.setDefaultFirmName(controller.Firm);

    if (Get.arguments != null) {
      isUpdate.value = true;
      YarnSaleController controller = Get.find();
      var item = YarnSaleModel.fromJson(Get.arguments['item']);
      idController.text = '${item.id}';
      billNoController.text = '${item.billNO}';
      salesDateController.text = item.salesDate ?? '';
      detailsController.text = item.details ?? '';
      freightController.text = '${item.freight}';
      lrNoController.text = item.lrNo ?? '';
      lrController.text = item.lrDate ?? '';
      destinationNameController.text = item.destinationName ?? '';
      transportNameController.text = item.transport ?? '';
      ...
      var customerList = controller.Customer.where(
          (element) => '${element.id}' == '${item.customerId}').toList();
      ...
      var accountTypeList = controller.purchaseAccountDropdown
          .where((element) => '${item.salesAno}' == '${element.id}')
          .toList();
      ...
      item.itemDetails?.forEach((element) {
        amount.value = double.tryParse('${element.amount}') ?? 0;
  ...
  Widget yarnSaleTable() {
    return MySFDataGridItemTable(
      ...
      onRowSelected: (index) async {
        ...
        var result = await Get.to(const AddYarnSaleBottomSheet(),
            arguments: {'item': item});
        if (result != null) {
