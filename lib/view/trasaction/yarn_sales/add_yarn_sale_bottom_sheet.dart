...
class AddYarnSaleBottomSheet extends StatefulWidget {
  const AddYarnSaleBottomSheet({super.key});

  @override
  State<AddYarnSaleBottomSheet> createState() => _State();
}

class _State extends State<AddYarnSaleBottomSheet> {
...
//  TextEditingController quantityController = TextEditingController();
 // TextEditingController lessController = TextEditingController(text: '0');
  TextEditingController netQuantityController = TextEditingController(text: "0");
  ...
  submit() {
    if (_formKey.currentState!.validate()) {
      var request = {
        ...
        "less_quantity": 0,
        "gross_quantity": double.tryParse(netQuantityController.text) ?? 0,
  ...
  void _initValue() {
    if (Get.arguments != null) {
      ...
      // lessController.text = tryCast(item['less_quantity']);
      ...
      deliveredFromController.text = stockIn;
      var yarnList = controller.Yarn.where(
          (element) => '${element.id}' == '${item['yarn_id']}').toList();
      if (yarnList.isNotEmpty) {
        yarnName.value = yarnList.first;
  ...
  Future<void> yarnStockBalanceCheck() async {
    ...
    for (var element in ll) {
      saleQty += element['gross_quantity'];
    }

    stockBalanceController.text = (stockQty - saleQty).toStringAsFixed(3);
  }
}
