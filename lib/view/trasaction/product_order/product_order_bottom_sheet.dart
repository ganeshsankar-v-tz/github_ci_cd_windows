...
class AddProductOrderBottomSheet extends StatefulWidget {
  final ProductOrderItemDataSource dataSource;

  const AddProductOrderBottomSheet({
    super.key,
    required this.dataSource,
  });

  @override
  State<AddProductOrderBottomSheet> createState() =>
      _WeavingItemBottomSheetState();
}

class _WeavingItemBottomSheetState extends State<AddProductOrderBottomSheet> {
  ...
  ProductOrderController controller = Get.find();
    ...
    return GetBuilder<ProductOrderController>(builder: (controller) {
      return ShortCutWidget(
        appBar: AppBar(
          ...
          title: const Text('Add Item Product Order'),
        ),
  ...
  _productStockDetails(int? id) async {
    int orderQty = 0;
