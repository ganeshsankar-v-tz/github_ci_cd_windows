...
class ProductSale extends StatefulWidget {
  ...
  static const String routeName = '/product_sale_list';

  @override
  State<ProductSale> createState() => _State();
}

class _State extends State<ProductSale> {
  ProductSaleController controller = Get.put(ProductSaleController());
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ProductSaleController>(builder: (controller) {
      ...
      return CoreWidget(
        appBar: AppBar(
          title: const Text('Transaction / Product Sale'),
          actions: [
            Visibility(
              visible: false,
              child: TextButton(
        ...
        child: MySFDataGridRawTable(
          ...
          columns: [
            ...
            GridColumn(
              width: 160,
              columnName: 'order_no',
              label: const MyDataGridHeader(title: 'Order No'),
            ),
            GridColumn(
            ...
            GridColumn(
              ...
              label: const MyDataGridHeader(
                  alignment: Alignment.center, title: 'Net Amount'),
            ),
          ],
  ...
  void _api({var request = const {}}) async {
    var response = await controller.productSaleList(request: request);
    list.clear();
  ...
  void _add({Map<String, dynamic>? args}) async {
    var result = await Get.toNamed(AddProductSales.routeName, arguments: args);
    if (result == 'success') {
      _api(request: controller.filterData ?? {});
    }
  }
  ...
  void _filter() async {
    final result = await showDialog(
      context: context,
      builder: (_) => const ProductSaleFilter(),
    );
  ...
  void updateDataGridRows() {
    _employeeData = _list.map<DataGridRow>((e) {
      return DataGridRow(cells: [
        ...
        DataGridCell<dynamic>(columnName: 'id', value: e['e_date']),
        ...
        DataGridCell<dynamic>(
            columnName: 'total_amount', value: e['net_total']),
      ]);
    }).toList();
  }
