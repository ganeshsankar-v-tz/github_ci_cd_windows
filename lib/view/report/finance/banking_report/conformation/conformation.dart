import 'package:abtxt/view/report/finance/banking_report/conformation/conformation_details.dart';
...
class ConformationList extends StatefulWidget {
  const ConformationList({super.key});

  static const String routeName = '/conformation_list';
...
class _ConformationListState extends State<ConformationList> {
  ...
  void _api() async {
    var response = await controller.userConformationListApi();
    list.clear();
  ...
  void _add({Map<String, dynamic>? args}) async {
    var result =
        await Get.toNamed(ConformationItemDetails.routeName, arguments: args);
    if (result == 'success') {
      _api();
    } else {
...
class MyDataSource extends DataGridSource {
  ...
  void updateDataGridRows() {
    _employeeData = _list.map<DataGridRow>((e) {
      ...
      var status = "";
      if (e["status"] == "Waiting_for_confirmation") {
