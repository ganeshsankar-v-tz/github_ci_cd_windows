...
import 'package:abtxt/view/report/finance/banking_report/po_details/po_update.dart';
...
class PoList extends StatefulWidget {
  ...
  static const String routeName = '/po_list';

  @override
  State<PoList> createState() => _PoListState();
}
...
class _PoListState extends State<PoList> {
    ...
    return GetBuilder<BankingReportController>(
      builder: (controller) {
        return ShortCutWidget(
          appBar: AppBar(
            title: const Text("Po List - Banking Report"),
            actions: const [],
          ),
  ...
  void _api({var request = const {}}) async {
    var response = await controller.poList(request: request);
    list.clear();
  ...
  void _add({Map<String, dynamic>? args}) async {
    var result =
        await Get.toNamed(PoDetails.routeName, arguments: args);
    if (result == 'success') {
      _api();
    }else {
...
class MyDataSource extends DataGridSource {
  ...
  void updateDataGridRows() {
    _employeeData = _list.map<DataGridRow>((e) {
      ...
      var status = "";
      if (e["status"] == "Waiting_for_po_confirmation") {
    ...
    return DataGridRowAdapter(
      ...
      if (e.columnName == "button") {
        return Padding(
          ...
          child: Visibility(
            visible: row.getCells()[8].value == "Waiting For Transaction",
