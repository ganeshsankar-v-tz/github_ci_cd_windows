...
import 'add_dyer_yarn_opening_balance.dart';
import 'dyer_yarn_opening_balance_controller.dart';

List<DyerYarnOpeningBalanceModel> paginatedOrders = [];
DyerYarnDataSource dataSource = DyerYarnDataSource();
int totalPage = 1;
int _rowsPerPage = 10;

class DyerYarnOpeningBalance extends StatefulWidget {
  const DyerYarnOpeningBalance({Key? key}) : super(key: key);
  static const String routeName = '/DyerYarnOpeningBalanceList';

  @override
  State<DyerYarnOpeningBalance> createState() => _State();
}

class _State extends State<DyerYarnOpeningBalance> {
      ...
      return KeyboardWidget(
        bindings: [
          ...
          KeyAction(
            ...
            'Add', () => Get.toNamed(AddDyerYarnOpeningBalance.routeName),
            isControlPressed: true,
          ),
        ],
        child: Scaffold(
          appBar: AppBar(
            title: const Text('Basic Info / Dyer Yarn Opening Balance'),
            centerTitle: false,
            elevation: 0,
            actions: [
              ElevatedButton.icon(
                onPressed: () async {
                  var item =
                      await Get.toNamed(AddDyerYarnOpeningBalance.routeName);
...
class DyerYarnDataSource extends DataGridSource {
  DyerYarnDataSource() {
          ...
          if (e.columnName == 'button') {
            return MyEditDelete(
              ...
              onDeletePressed: () async {
                ...
                await controller.deleteDyer(id);
                dataSource.notifyListeners();
              },
            );
          } else {
  ...
  Future<bool> handlePageChange(int oldPageIndex, int newPageIndex) async {
    if (newPageIndex < totalPage) {
      DyerYarnOpeningBalanceController controller = Get.find();
      var result = await controller.Dyer(
