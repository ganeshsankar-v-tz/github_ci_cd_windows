...
import 'package:abtxt/view/basics/saree_checker/saree_checker_controller.dart';
...
import 'add_saree_checker.dart';

class SareeChecker extends StatefulWidget {
  const SareeChecker({super.key});

  static const String routeName = '/saree_checker';

  @override
  State<SareeChecker> createState() => _State();
}

class _State extends State<SareeChecker> {
  SareeCheckerController controller = Get.find();
  ...
  Widget build(BuildContext context) {
    return GetBuilder<SareeCheckerController>(builder: (controller) {
      return CoreWidget(
        appBar: AppBar(
          title: const Text('Basic Info / Saree Checker'),
          actions: [
  ...
  void _add({Map<String, dynamic>? args}) async {
    var result = await Get.toNamed(AddSareeChecker.routeName, arguments: args);
    if (result == 'success') {
      _api();
    }
  }
