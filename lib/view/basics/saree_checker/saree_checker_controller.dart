...
class SareeCheckerController extends GetxController with StateMixin {
  @override
  void onInit() {
  ...
  Future<List<SareeCheckerModel>> checkerDetails(
    ...
    await HttpRepository.apiRequest(
            HttpRequestType.get, 'api/saree_checker_list?$query')
        .then((response) {
      change(null, status: RxStatus.success());
      if (response.success) {
  ...
  void add(var request) async {
    change(null, status: RxStatus.loading());
    await HttpRepository.apiRequest(
            HttpRequestType.post, 'api/saree_checker_add',
  ...
  void edit(var request) async {
    change(null, status: RxStatus.loading());
    await HttpRepository.apiRequest(
            HttpRequestType.post, 'api/saree_checker_update',
            requestBodydata: request)
        .then((response) {
      change(null, status: RxStatus.success());
      if (response.success) {
  ...
  Future<dynamic> delete(var id, var password) async {
    ...
    await HttpRepository.apiRequest(HttpRequestType.delete,
            'api/saree_checker_delete?id=$id&password=$password')
        .then((response) {
      change(null, status: RxStatus.success());
      if (response.success) {
        var json = jsonDecode(response.data);
