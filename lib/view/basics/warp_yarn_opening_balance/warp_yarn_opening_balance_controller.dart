...
class WarpYarnOpeningBalanceController extends GetxController with StateMixin {
...
//yarn name
  ...
  Future<dynamic> Warp({var page = "1",var limit = "10"}) async {
    change(null, status: RxStatus.loading());
    dynamic result = [];
    await HttpRepository.apiRequest(
            HttpRequestType.get, 'api/warperopening?page=$page&limit=$limit')
        .then((response) {
      if (response.success) {
        var json = jsonDecode(response.data);
        var data = json['data']['data'];
        var list = (data as List)
            .map((i) => WarpYarnOpeningBalanceModel.fromJson(i))
            .toList();
        ...
        result = {"list": <WarpYarnOpeningBalanceModel>[], "totalPage": 1};
      }
    });
  ...
  void addwarp(var request) async {
    await HttpRepository.apiRequest(HttpRequestType.post, 'api/addWarperOpen',
  ...
  void updatewarp(var request, var id) async {
    await HttpRepository.apiRequest(
            HttpRequestType.upload, 'api/updateWarperOpen/$id',
            formDatas: request)
        .then((response) {
      if (response.success) {
        var json = jsonDecode(response.data);
  ...
  Future<dynamic> deletewarp(var id) async {
    await HttpRepository.apiRequest(
            HttpRequestType.delete, 'api/warperopening/$id')
        .then((response) {
      if (response.success) {
        var json = jsonDecode(response.data);
        AppUtils.showSuccessToast(message: "${json["message"]}");
