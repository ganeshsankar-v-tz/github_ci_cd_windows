...
import 'add_new_color.dart';
import 'colour_filter.dart';
import 'new_color_controller.dart';

class NewColor extends StatefulWidget {
  const NewColor({Key? key}) : super(key: key);
  static const String routeName = '/color';

  @override
  State<NewColor> createState() => _State();
}

class _State extends State<NewColor> {
  NewColorController controller = Get.put(NewColorController());
  ...
  Widget build(BuildContext context) {
    return GetBuilder<NewColorController>(builder: (controller) {
      ...
      return CoreWidget(
        appBar: AppBar(
          title: const Text('Basic Info / New Color'),
          actions: [
            MyRefreshIconButton(
  ...
  void _api({var request = const {}}) async {
    var response = await controller.colors(request: request);
    list.clear();
  ...
  void _filter() async {
    final result = await showDialog(
      context: context,
      builder: (_) => const NewColorFilter(),
    );
