...
class WeavingBottomSheet extends StatefulWidget {
  const WeavingBottomSheet({super.key});

  @override
  State<WeavingBottomSheet> createState() => _WeavingItemBottomSheetState();
}

class _WeavingItemBottomSheetState extends State<WeavingBottomSheet> {
  ...
  var entryTypeFocusNode = FocusNode();
  ...
  Rxn<NewColorModel> warpDeliveryColour = Rxn<NewColorModel>();
  ...
  TextEditingController yarnDeliveryDeliveryFrom = TextEditingController();
  TextEditingController yarnDeliveryBoxNo = TextEditingController();
  TextEditingController yarnDeliveryPack = TextEditingController();
  ...
  TextEditingController goodsInwardDamaged = TextEditingController();
  TextEditingController goodsInwardDetails = TextEditingController();
  TextEditingController goodsInwardPending = TextEditingController();
  ...
  TextEditingController returnYarnStockTo = TextEditingController();
  ...
  TextEditingController messageLanguage = TextEditingController();
  TextEditingController messageMsgType = TextEditingController();
                  ...
                  Flexible(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: Colors.white,
                        border: Border.all(
                            color: const Color(0xFFF9F3FF), width: 12),
                      ),
                      child: Form(
                        key: _formKey,
                        child: Column(
                            ...
                            Wrap(
                              children: [
                                ...
                                MyAutoComplete(
                                  ...
                                  onChanged:
                                      (WeavingEntryTypeModel item) async {
                                    _selectedEntryType.value = item;
                                    //FocusManager.instance.primaryFocus?.nextFocus();
                                    await _apiCale(item.entryType);
                                  },
                                ),
                              ],
                            ),
                ...
                Focus(
                  ...
                  child: MyAutoComplete(
                    ...
                    onChanged: (WarpType item) {
                      ...
                      int subWeaverNo = controller.request['loom_id'];
            ...
            Wrap(
              children: [
                MyAutoComplete(
                  ...
                  isValidate: false,
                  selectedItem: warpDeliveryColour.value,
                  onChanged: (NewColorModel item) {
                    warpDeliveryColour.value = item;
                    warpDeliveryWarpColor.text = "${item.name}, ";
                  },
                ),
                MyTextField(
            ...
            Wrap(
              children: [
                ...
                Focus(
                  skipTraversal: true,
                  child: MyTextField(
                    // enabled: transferAmountCredit.text != "0",
  ...
  _initValue() async {
    ...
    /// Warp Delivery
    warpDeliveryEmptyType.text = Constants.emptyType[0];

    /// Yarn Delivery
    yarnDeliveryDeliveryFrom.text = Constants.deliveredFrom[0];
    yarnDeliveryPack.text = "0";

    /// Goods Inward
    goodsInwardDamaged.text = Constants.Weight[0];
    goodsInwardPending.text = Constants.Weight[0];

    /// Payment
    paymentType.text = Constants.Tokes[0];

    ///  Return Yarn
    returnYarnStockTo.text = Constants.stockTo[0];

    /// Message
    messageLanguage.text = Constants.language[0];
    messageMsgType.text = Constants.messageType[0];

    var weavingId = Get.arguments['weaving_ac_id'];
  ...
  void transferWarp(var entryType) async {
    ...
    var weavingAcId = Get.arguments['weaving_ac_id'];
  ...
  submit() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (entryType == "Warp Delivery") {
        ...
        request["warp_type"] = warpDeliveryWarpType.text;
        ...
        request["credit"] = double.tryParse(goodsInwardAmount.text) ?? 0.0;
        ...
        request["debit"] = double.tryParse(paymentAmount.text) ?? 0.0;
        ...
        request["credit"] = double.tryParse(creditAmountRs.text) ?? 0.0;
        ...
        request["debit"] = double.tryParse(debitAmountRs.text) ?? 0.0;
        ...
        request["yarn_qty"] = double.tryParse(yarnWastageQuantity.text) ?? 0.0;
        ...
        request["meter"] = double.tryParse(warpExcessWarpMeter.text) ?? 0.0;
        ...
        if (warpType == "Main Warp") {
          if (warpBalanceQty >= qty) {
            ...
            AppUtils.showErrorToast(
                message: "Balance Warp Qty Less Then Warp Qty");
          }
        } else {
        ...
        if (transferWarpWarpDesign.value?.otherWarpId == null ||
            transferWarpWarpDesign.value!.otherWarpId!.isEmpty) {
        ...
        request["we_details"] = openBalDetails.text;
        Get.back(result: request);
      } else if (entryType == "Inward - Cops, Reel") {
        /*request["weaver_no"] = ICR_Weav_no.text;
        request["weav_no"] = ICR_challan_no.text;
        request["bm_in"] = ICR_beamIn.text;
        request["bm_out"] = ICR_beamOut.text;
        request["bbn_in"] = ICR_bobbinIN.text;
        request["bbn_out"] = ICR_bobbinOut.text;
        request["cops_in"] = ICR_copsIN.text;
        request["cops_out"] = ICR_copsOut.text;
        request["reel_in"] = ICR_reelIN.text;
        request["reel_out"] = ICR_reelOut.text;*/
        Get.back(result: request);
      } else if (entryType == "O.Bal - Amount") {
        request["debit"] = double.tryParse(openBalAmountDebit.text) ?? 0.0;
        request["credit"] = double.tryParse(openBalAmountCredit.text) ?? 0.0;
        ...
        request["warp_type"] = openBalWarpType.text;
        request["meter"] = double.tryParse(openBalWarpMetre.text) ?? 0.00;
        ...
        request["yarn_qty"] = double.tryParse(openBalYarnQty.text) ?? 0.0;
        ...
        request["warp_qty"] = qty;
        request["meter"] = double.tryParse(warpDropMetre.text) ?? 0.0;
        ...
        request["other_warpid"] = warpDropWarpDesign.value?.warpDesignId;
        var emptyType = warpDropEmptyType.text;
        if (emptyType == "Beam") {
          request["bm_in"] = int.tryParse(warpDropEmptyQty.text) ?? 0;
        } else {
          request["bbn_in"] = int.tryParse(warpDropEmptyQty.text) ?? 0;
        }
        Get.back(result: request);
      }
    }
