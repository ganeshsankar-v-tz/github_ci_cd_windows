...
import 'package:abtxt/view/production/warp_or_yarn_delivery/warp_or_yarn_delivery_bottom_sheet.dart';
...
class AddWarporYarnDeliveryProduction extends StatefulWidget {
  const AddWarporYarnDeliveryProduction({super.key});

  static const String routeName = '/add_Warp_or_Yarn_Delivery_Production';

  @override
  State<AddWarporYarnDeliveryProduction> createState() => _State();
}

class _State extends State<AddWarporYarnDeliveryProduction> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<WarpOrYarnDeliveryController>(builder: (controller) {
      return ShortCutWidget(
        appBar: AppBar(
          title: Text(
              "${idController.text == '' ? 'Add' : 'Update'} Warp / Yarn Delivery"),
          actions: [
            Visibility(
  ...
  Future<void> _print() async {
    ...
    String? response = await controller.warpOrYarnDeliveryPdf(request: request);
    ...
    if (Get.arguments != null) {
      ...
      if (obj != null) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) async {
          ...
          // controller.id = recNo;
          // controller.challanNo = challanNo;
        ...
        var weaverId = Get.arguments["weaver_id"];
        var enable = Get.arguments["enable"];

        var result = await controller.WeaverNameInfo();
        var weaverList =
  ...
  bottomSheetNavigate() async {
    var result =
        await Get.to(() => const WarpOrYarnDeliveryProductionBottomSheet());
    if (result != null) {
      if (result['entry_type'] == 'Empty - (In / Out)') {
...
class ItemDataSource extends DataGridSource {
  ...
  void updateDataGridRows() {
    dataGridRow = _list.map<DataGridRow>((e) {
      ...
      var details = "";
      num meter = 0;
      double yarnQty = double.tryParse("${e["yarn_qty"]}") ?? 0;

      /// Warp Delivery
      if (e["entry_type"] == "Warp Delivery") {
        particulars = tryCast(e["warp_design_name"]);
        quantity = tryCast(e["warp_qty"]);
        meter = e["meter"];
        if (e["warp_type"] == "Other") {
          details =
              "${tryCast(e["warp_id"])}, Mr: ${tryCast(e["meter"])}, ${tryCast(e["product_details"])}";
        } else {
          details =
              "${tryCast(e["warp_id"])},\n ${tryCast(e["product_details"])}";
        }
      }
      ...
      if (e["entry_type"] == "Yarn Delivery") {
        particulars =
            "${tryCast(e["yarn_name"])} - ${tryCast(e["yarn_color_name"])}";
        details =
            "${yarnQty.toStringAsFixed(3)} Kgs, ${tryCast(e["product_details"])}";
      }
      ...
      if (e["entry_type"] == "Rtrn-Yarn") {
        particulars = "${e["yarn_name"]} - ${e["yarn_color_name"]}";
        if (e["yarn_empty_type"] == "Cops") {
          details =
              "${yarnQty.toStringAsFixed(3)} Kgs, C - ${e["cops_in"]}, ${e["product_details"]}";
        } else if (e["yarn_empty_type"] == "Reel") {
          details =
              " ${yarnQty.toStringAsFixed(3)} Kgs, R - ${e["reel_in"]}, ${e["product_details"]}";
        } else {
          details =
              "${yarnQty.toStringAsFixed(3)} Kgs,${tryCast(e["product_details"])}";
        }
      ...
      return DataGridRow(cells: [
        ...
        DataGridCell<dynamic>(columnName: 'qty', value: quantity),
        DataGridCell<dynamic>(columnName: 'meter', value: meter),
    ...
    return DataGridRowAdapter(
        ...
        cells: row.getCells().map<Widget>((c) {
          TextStyle? getTextStyle() {
            ...
            if (sync == 0) {
              if (c.columnName == "product_details") {
                return TextStyle(
