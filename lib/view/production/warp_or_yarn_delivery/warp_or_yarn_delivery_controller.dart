...
class WarpOrYarnDeliveryController extends GetxController with StateMixin {
  ...
  Future<List<WarpOrYarnDeliveryListModel>> WarporYarnDelivery(
    ...
    await HttpRepository.apiRequest(
      ...
      if (response.success) {
        ...
        list = list.where((element) {
          return !("${element.entryType}").contains("Empty - (In / Out)");
        }).toList();
      } else {
...
//   Weaver Name
  ...
  /// Pdf
  Future<String?> warpOrYarnDeliveryPdf({request = const {}}) async {
    change(null, status: RxStatus.loading());
    ...
    var url;
    await HttpRepository.apiRequest(
            HttpRequestType.get, 'api/warp_yarn_delivery_pdf?$query')
        .then((response) {
      change(null, status: RxStatus.success());
      var json = jsonDecode(response.data);
      if (response.success) {
                  ...
                  children: [
                    ...
                    OutlinedButton(
                      ...
                      onPressed: () async {
                        var request = {'id': id};
                        String? response =
                            await warpOrYarnDeliveryPdf(request: request);
  ...
  /// Weaver, Loom And Current Status By Get The Warp Delivered Status
  Future<bool?> loomWarpDeliveryStatus(
      var weaverId, var loomNo, var currentStatus) async {
    bool? result;
    await HttpRepository.apiRequest(HttpRequestType.get,
            'api/check_validation_main_warp?weaver_id=$weaverId&loom_no=$loomNo&current_status=$currentStatus')
        .then((response) {
      var json = jsonDecode(response.data);
      if (response.success) {
        var data = json['data'];
        mainWarpDeliverStatus = data;
      } else {
        print('error: ${json['message']}');
      }
    });
  ...
  Future<List<dynamic>> overAllWeftBalance(var weaverId, var loomNo) async {
    List<dynamic> result = [];
    await HttpRepository.apiRequest(HttpRequestType.get,
      ...
      if (response.success) {
        var data = json['data'];
        List<dynamic> list = data["weft_balance"];
        var localDetails =
            itemList.where((element) => element["sync"] == 0).toList();

        /// local added yarn quantity calculation
        /// if entry type = yarn delivery add the weaver stock and local yarn qty
        /// if entry type = return yarn minus the weaver stock and local yarn qty
        for (var e in localDetails) {
          if (e["entry_type"] == "Yarn Delivery") {
            for (int i = 0; i < list.length; i++) {
              var d = list[i];
              if (d["yarn_id"] == e["yarn_id"]) {
                var yarnQty =
                    (double.tryParse("${list[i]["wev_stock"]}") ?? 0) +
                        e["yarn_qty"];
                list[i]["wev_stock"] = yarnQty;
              }
            }
          } else if (e["entry_type"] == "Rtrn-Yarn") {
            for (int i = 0; i < list.length; i++) {
              var d = list[i];
              if (d["yarn_id"] == e["yarn_id"]) {
                var yarnQty =
